...
const float	FIRE_TIME		=  4.f;		// 敌方坦克发射炮弹的时间间隔
const float CREATE_TANK_TIME = 3.f;		// 每批次生成坦克的时间间隔
const float	GAME_TIME		=	60.f;	// 一局游戏的时间
const float	TANK_SPEED		 = 8.f;     // 敌方坦克速度
const float BULLET_SPEED	 = 15.f;		// 子弹速度
                   ...
                   int       nCmdShow)
{
    ...
    while( dEngineMainLoop() ) // 运行游戏引擎，刷新游戏屏幕。当引擎关闭时，返回false
    {
        ...
        {
            if(g_iScore>g_iHight)
            {
                g_iHight=g_iScore;
                dSetTextValue("Hight", g_iScore);
            }
            dStopAllSound();
            ...
            for(int i=0; i<g_iWallCount; i++)
            {
                ...
                sprintf(szName, "sea%d", i);
                dDeleteSprite(szName);
                sprintf(szName, "stock%d", i);
                dDeleteSprite(szName);
                sprintf(szName, "grass%d", i);
                dDeleteSprite(szName);
            }
            g_iWallCount = 0;
            bLoad = true;
...
void dOnKeyDown( const int iKey, const int iAltPress, const int iShiftPress, const int iCtrlPress )
{
    ...
    if(iKey == KEY_SPACE && g_bStart == false) // 游戏未开始，按下空格键
    {
        ...
        dPlaySound("paizi", 0, 1);
        dPlaySound("The_Mass", 1, 1);// 游戏开始，播放背景音乐
        dSetTextValue("time", (int)g_fGameTime);// 将我方坦克放在第11行第5列
...
void dOnSpriteColSprite( const char *szSrcName, const char *szTarName )
{
    ...
    if(p1 != NULL && IsDead(szTarName) && (strstr(szTarName,"tank") != NULL && strstr(szSrcName,"bullet") != NULL))
    {
        dDeleteSprite(szTarName);
        DeleteNode(szTarName);
    }
    if(p0 != NULL && IsDead(szSrcName) && strstr(szSrcName, "bullet") != NULL && strstr(szTarName, "sea") == NULL)
    {
...
void	OnTankColOther(const char* szTankName, const char* szOtherName)
{
	...
	if(strstr(szOtherName, "tank") != NULL)
	{
		...
		if(fSpeedX0 > 0.0001)
		{
			...
			if(fSpeedX1 < -0.0001)
            {
                dSetSpriteLinearVelocity(szOtherName, -TANK_SPEED, 0);
                dSetSpriteRotation(szOtherName, 90);
            }
		}
		else
		{
		...
		if(fSpeedY0 > 0.0001)
		{
			...
			if(fSpeedY1 < -0.0001)
            {
                dSetSpriteLinearVelocity(szOtherName, 0, TANK_SPEED);
                dSetSpriteRotation(szOtherName, 180);
            }
		}
		else
		{
